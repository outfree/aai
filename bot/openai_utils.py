...
class ChatGPT:
    ...
    async def send_message(self, message, dialog_messages=[], chat_mode="title_creator"):
    ...
    async def send_message_stream(self, message, dialog_messages=[], chat_mode="title_creator"):
        if chat_mode not in config.chat_modes.keys():
            raise ValueError(f"Chat mode {chat_mode} is not supported")
...
async def transcribe_audio(audio_file, response_format="json"):
    r = await openai.Audio.atranscribe("whisper-1", audio_file, response_format=response_format)
